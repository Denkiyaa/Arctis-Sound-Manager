...
## [1.3.1b1] - 19 August 2026

A pre-release: everything here is a fix found by running 1.3.0's Clips on real
hardware for a few days, and none of it has been through anyone else's machine
yet.

### Fixed

- **Watching a clip could lock the whole application up.** The window would not close and the desktop called it *not responding*; left alone overnight it ended as a crash. The stacks name both halves: the GUI thread holds the interpreter lock, because it is running Python, and wants a Qt mutex; the ffmpeg backend's audio-renderer threads hold that mutex and want the interpreter lock. Neither can give way — 38 of 72 threads asleep on a futex, 589 MB, no window. The renderer thread only ever needed the interpreter because Qt's ffmpeg backend drives the playback clock from there, so a Python slot on `positionChanged` forced it into the interpreter on that thread. The playhead is now read from a 50 ms timer on the GUI thread instead, which removes the only reason a decoder thread enters Python at all, and with it any chance of the two locks being taken in opposite orders.
- **The editor was never freed, and an evening of clips ended in a crash.** Opening a clip parented its dialog to the page, so `exec()` returning freed nothing: the video player, one player per channel and a decoder thread behind each stayed alive, and every clip opened left another set. Counted across four open/close cycles it was 1, 2, 3, 4 editors alive; it is now 1, 1, 1, 1. The channel mixer hands its players back rather than only clearing the list it holds them in, and does so with a deferred delete, because the release runs from the close event — which is the stack the crash dump was stopped in.
- **A clip could record a window picked weeks ago, and say nothing about it.** A clip named after the game, showing a file manager. Both halves were working as built: the name comes from the audio on the Game channel, the picture from the source the portal was told to restore — and that source was a window chosen once, made permanent, and never offered up for review. Wayland never tells the application what the source is, so the interface could not have said. Saved tokens now record what they were made for and a token that carries nothing is not restored, and the status line says when a capture replayed a remembered source rather than asking: *source: the one picked before (Change… to re-pick)*. Windows are still offered — what this records is games, and a game is a window.
- **The screen picker came back every few seconds while a game was running.** Declining it left no capture, and the poll starts one whenever a game is up and there is none — so what is meant to be one question became a dialog stealing focus for as long as the game ran, with no way to answer it except to give in. An automatic start now gets one attempt per game, and the block is lifted by the two things that mean the question is live again: the game going away, or Start being pressed. It surfaced from the tray unit rename, which is worth writing down because nothing about the symptom points at it — the portal keys its saved token to the application id, the id changed, and an answer already given stopped applying.
- **Opening the app while a game was running put a system dialog on top of the game.** Following the game called the portal whether or not a screen had ever been chosen, and with nothing saved the portal has to ask. An automatic start now requires a saved source: with one it starts in silence, without one the page says a game is running and no screen has been picked, and stops there. The picker appears when Start is pressed, which is a person asking for it.
- **Stop did not mean stopped.** Pressing it while a game was running bought a few seconds, because the poll saw no capture and started another. The button did what it said and the timer undid it, which is worse than the button not being there. A manual stop now blocks the automatic start until the game ends or Start is pressed.
- **Previewing a clip played into the headset while you were listening on something else.** The channel players were built with the system default output, and on a machine running this app the default is deliberately the headset — so browsing clips on Bluetooth earbuds was silent, with nothing on screen to explain it. The preview now follows the same remembered output the Output channel does, and sets a device only when there is a reason to: nothing remembered, or the remembered device already being the default, both mean *no opinion*.
- **The tray icon opened ten windows if you clicked it ten times.** Building the main window takes seconds, and the guard against a second one checked an attribute that is not set until the build finishes, so every click landing mid-build started another. Nobody clicks a tray icon once when nothing happens.
- **Under KDE the tray menu drew correctly and did nothing when clicked.** The menu was rebuilt from `aboutToShow`, which is exactly when Plasma is exporting it over DBusMenu — clearing it there leaves the exporter serialising actions it was never given ids for, thirteen in one session's journal, and every entry it then draws points at nothing. Exit included. The rebuild now happens when a status arrives and the menu is closed, and a status that lands while it is open is held rather than dropped.
- **Upgrading the Arch package restarted a service that no longer exists.** The unit rename left the install hook behind, still restarting `arctis-gui.service`, so `pacman -U` replaced the files and restarted nothing: the tray went on running the code it had loaded at startup, and every fix in the upgrade looked like it had not worked. Both names are listed now — the hook only restarts what is already running, so the old one is harmless on a machine that has moved on and is the only thing there on one that has not.
